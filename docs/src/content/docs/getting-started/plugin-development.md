...
3. **A valid plugin `package.json`.** The dev server validates the same
   ...
   [Plugin Distribution & Packing](../architecture/plugin-distribution.md#anatomy-of-an-installable-plugin)
   for required fields.
...
[required fields table](../architecture/plugin-distribution.md#required-packagejson-fields).
