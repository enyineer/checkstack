...
If you only want to *consume* a plugin as a platform operator, the
[Plugin Manager UI](#install-via-the-plugin-manager-ui) section at the
bottom is enough.
...
For platform operators, not authors:
...
4. Click **Preview install**. The platform fetches metadata and runs the
   compatibility check. The confirmation modal shows what will be
   installed, security warnings, and any blockers.
5. Type the plugin name to confirm. Install proceeds; the page refreshes
   to the Installed Plugins list when complete.

Failures surface inline in the modal (not the dreaded "Internal Server
Error") — typical messages:
...
Audit log of every install/uninstall (including in-flight failures across
multi-instance deployments) is at **Plugin Manager → Events**.
...
- [Plugin Architecture Overview](./plugin-system.md) — how plugins fit
  into Checkstack at runtime
- [Backend Plugin Development](../backend/plugins.md) — writing the
  `-backend` package
- [Frontend Plugin Development](../frontend/plugins.md) — writing the
  `-frontend` package
- [`docs/examples/plugin-release.yml`](../examples/plugin-release.yml) —
  GitHub Actions release workflow template
- [Runtime plugin system design](../plans/2026-05-03-runtime-plugin-system.md) —
  the design doc covering source resolution, bundle resolution, the
  artifact store, and multi-instance install/uninstall coordination
