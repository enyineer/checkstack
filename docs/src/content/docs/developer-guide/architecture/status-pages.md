...
Fan-out (`SubscriberService.notifyForSystems`) is driven by the notification platform's external-audience sink (`NotificationAudienceEvent`), which carries the affected `systemIds`, the source `sourcePluginId`, and - for a per-environment health transition - the `originEnvironmentId` it happened in. For each published, public, email-enabled page, the fan-out asks every scoping widget for its CURRENT effective system scope via the widget's own `resolveScopedSystems` - the SAME live expansion the widget renders from - so a page can never email about a system it does not surface. status-page-backend never imports the catalog; the owning domain plugin supplies the expansion.

Two further gates make delivery reflect exactly what the author placed on the page:

- **Per-category.** Only widgets whose `subscriptionCategory` matches the notification contribute to the surfaced set. A health status change reaches a page only through a HEALTH widget that shows the affected system, an incident only through an incident widget, and so on - so a page that lists a system's incidents but never its health does NOT email health subscribers about it. Health widgets (`banner`, `systemHealth`, `groupStatus`, `uptime`) declare `subscriptionCategory: "health"`; the incident and maintenance widgets declare their own. An uncategorized source falls back to every scoping widget (reaching only legacy NULL-categories subscribers).
- **Per-environment origin.** When the notification carries an `originEnvironmentId`, a page publishing a SPECIFIC environment set is skipped unless it publishes that environment - so a `development` failure never reaches a prod-only page's subscribers, even for a system also shown in prod. A page publishing ALL environments, and any env-less source (incident, maintenance, whole-system health rollup), skip this gate. This is stricter than the display-time [multi-environment caveat](#environment-scoping): a system in several environments is SHOWN on a page publishing any of them, but its per-environment health only EMAILS the pages that publish the environment the change occurred in.
