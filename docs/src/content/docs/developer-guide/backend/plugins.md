---
title: "Backend Plugin Development Guide"
description: "Scaffold a backend plugin: oRPC contract implementation, Drizzle schemas, Zod validation, and end-to-end type safety."
---
...
> **Note:** Don't forget to also create the corresponding `-common` package to define your contract. See [Common Plugin Guidelines](/checkstack/developer-guide/common/plugins/) for details.
...
> **See also:** For resource-level access control (restricting access to specific systems, health checks, etc.), see [Teams and Resource-Level Access Control](/checkstack/developer-guide/backend/teams/).
...
Contracts are defined in the `-common` package using `@orpc/contract`. See [Common Plugin Guidelines](/checkstack/developer-guide/common/plugins/) for details.
...
- **Plugin Configuration**: Use `ConfigService` for settings that control plugin behavior
  ...
  - See [Configuration Storage Guide](/checkstack/developer-guide/backend/config-service/) for details
...
See [Drizzle Schema Isolation](/checkstack/developer-guide/backend/drizzle-schema/) for details.
