...
- **Rule.** [State management & horizontal scale](https://github.com/enyineer/checkstack/blob/main/.claude/rules/state-and-scale.md) is the principle: a reactive entity's `read` MUST resolve from shared, durable storage (the plugin's own Postgres tables, or a derivation of them), never from process-local memory. In-memory is allowed only for genuinely pod-local infrastructure that is never the queryable source of truth (a live-socket registry), marked `declareNonReactiveState`.
