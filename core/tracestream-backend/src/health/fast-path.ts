 ...
 * error spans for a stream, we enqueue a DEBOUNCED one-off evaluation for every
 * enabled assignment SLICE (per effective environment) of every enabled
 * tracestream configuration that references the stream - so an error burst is
 * reflected in health within seconds instead of at the next scheduled tick.
 *
 ...
 * (`HealthCheckApi`), cached 60s per stream. Mirrors logstream's fast-path.
 *
 ...
 *     Redis round-trip.
 *  2. A pod-local attempted-bucket map collapses repeat flushes: once a slice has
 *     been enqueued in a given 15s debounce bucket, further flushes in the same
 *     bucket do ZERO queue work.
 */
...
import {
  TRACESTREAM_QUALIFIED_STRATEGY_ID,
  TRACESTREAM_FAST_PATH_PREFIX,
} from "./constants";
import type { IngestFlushInfo } from "./setup";

/**
 * An enabled assignment of a tracestream configuration to a system, resolved
 * down to the effective environment slices the executor will accept.
 * `environmentIds` is `[null]` for an env-less system (a single env-less run),
 * else the effective environment ids.
 */
 ...
 * never blocks the flush loop). `whenIdle` is a TEST affordance: it resolves once
 * every detached fan-out scheduled so far has settled.
 */
...
/** Whether a flush warrants a fast-path evaluation (any error spans committed). */
export function shouldFastPath(errorSpanCount: number): boolean {
...
class AttemptedBuckets {
  ...
  /**
   * Mark `key` attempted for `bucket`. Returns true when this is the FIRST attempt
   * in that bucket (caller should enqueue), false when already attempted (skip).
   */
...
/**
 * Build the fast-path hook from its collaborators. Kept collaborator-injected so
 * the fan-out + debounced jobId can be unit-tested with a mocked resolver and
 * enqueue.
 */
export function createFastPath({
  ...
  const fanOut = async ({ streamId }: IngestFlushInfo): Promise<void> => {
    ...
    // Mark every not-yet-attempted slice SYNCHRONOUSLY (no await between marks) so
    // two concurrent fan-outs for the same stream+bucket cannot both enqueue.
    const toEnqueue: {
      payload: HealthCheckJobPayload;
      jobId: string;
      key: string;
    ...
    for (const assignment of assignments) {
      for (const environmentId of assignment.environmentIds) {
        ...
        toEnqueue.push({
          ...
          jobId: fastPathJobId({
            prefix: TRACESTREAM_FAST_PATH_PREFIX,
            configId: assignment.configId,
            systemId: assignment.systemId,
            environmentId,
            nowMs,
    ...
    logger.debug(
      `tracestream fast-path: enqueued ${toEnqueue.length} evaluation(s) for stream ${streamId}`,
    );
  };

  const hook = ((info: IngestFlushInfo): void => {
    if (!shouldFastPath(info.errorSpanCount)) return;
    ...
    const work = fanOut(info)
      .catch((error) => {
        logger.warn(
          `tracestream fast-path failed for stream ${info.streamId}: ${String(error)}`,
        );
      })
      .finally(() => {
        pending.delete(work);
 ...
 * cross-plugin RPC client. Enumerates tracestream configurations, filters to
 * those referencing the stream (and not paused), collects their enabled
 * assignments, and resolves each assignment's effective environment ids via the
 * sanctioned `resolveEnqueueEnvironments` procedure. Cached 60s per stream (a 60s
 * staleness is acceptable - the scheduled tick covers anything the cache misses).
 */
    ...
    cache.wrap(
      ...
      async () => {
        ...
        const matching = configurations.filter(
          (c) =>
            c.strategyId === TRACESTREAM_QUALIFIED_STRATEGY_ID &&
            !c.paused &&
