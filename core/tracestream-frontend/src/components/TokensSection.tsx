...
export function TokensSection({
  ...
  const [revokeTarget, setRevokeTarget] = useState<TraceStreamToken | null>(
    null,
  );
      ...
      <CardContent>
        {isLoading ? (
          ...
          <EmptyState
            ...
            title="No tokens yet"
            description="Mint a source token to start pushing spans to this stream."
            actions={
              canManage ? (
                <Button onClick={() => setMintOpen(true)}>
                  <Plus className="size-4" />
      ...
      <ConfirmationModal
        ...
        message={
          <>
            ...
            token will immediately stop being able to send spans. This cannot be
            undone.
          </>
        }
        confirmText="Revoke"
