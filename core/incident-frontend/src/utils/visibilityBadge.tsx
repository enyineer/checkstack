...
/**
 * A small chip marking a non-public incident update/hotlink. Renders nothing for
 * `public` items (the default, no chip needed) so the timeline/link list stays
 * quiet; `logged_in` and `internal` items get an eye / lock glyph plus a label
 * so the restricted audience reads by icon + words, not color alone.
 *
