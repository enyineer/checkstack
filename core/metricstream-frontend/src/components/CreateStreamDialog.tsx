...
import { MetricstreamApi, metricstreamAccess } from "@checkstack/metricstream-common";
...
export function CreateStreamDialog({
  ...
  const { allowed: allowGlobal } = accessApi.useAccess(metricstreamAccess.manage);
  ...
  const createMutation = client.createStream.useGatedMutation({
    onSuccess: (stream) => {
      ...
      reset();
      onOpenChange(false);
      navigate(`/metricstream/${stream.id}`);
    },
    onError: (error) => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent size="default">
        <DialogHeader>
          <DialogTitle>New metric stream</DialogTitle>
          <DialogDescription>
            A stream receives your metrics over OTLP, native JSON or Prometheus
            scraping. Mint a source token or add a scrape target after creating
            it to start ingesting.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="metricstream-name">Name</Label>
            <Input
              id="metricstream-name"
              value={name}
              onChange={(e) => {
          ...
          <div className="space-y-1.5">
            <Label htmlFor="metricstream-description">Description</Label>
            <Textarea
              id="metricstream-description"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Application metrics from the checkout service"
              rows={2}
              maxLength={2000}
            />
          </div>
