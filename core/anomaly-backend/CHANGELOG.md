...
- 43e4484: Kill the N+1 query loops that made anomaly-backend the hottest ongoing scoped-db
  ...
  - The hourly baseline analyzer:
    - Preloads BOTH per-assignment config reads (`getAnomalyConfigsByIds` +
      ...
      single `withScopedTransaction`, replacing `2 * N` standalone `SELECT`s with 2. The `getRunsForAnalysis` RPC stays OUTSIDE the transaction.
