...
export function CreateStreamDialog({
  ...
  const { allowed: allowGlobal } = accessApi.useAccess(logstreamAccess.manage);
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent size="default">
        <DialogHeader>
          <DialogTitle>New log stream</DialogTitle>
          <DialogDescription>
            A stream receives your logs over OTLP, native JSON or syslog. Mint a
            source token after creating it to start shipping.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="logstream-name">Name</Label>
            <Input
              id="logstream-name"
              value={name}
              onChange={(e) => {
          ...
          <div className="space-y-1.5">
            <Label htmlFor="logstream-description">Description</Label>
            <Textarea
              id="logstream-description"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Application logs from the checkout service"
              rows={2}
              maxLength={2000}
            />
          </div>
