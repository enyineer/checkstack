...
import { LogstreamApi, type LogStreamToken } from "@checkstack/logstream-common";
...
export function TokensSection({
  ...
  const [revokeTarget, setRevokeTarget] = useState<LogStreamToken | null>(null);
      ...
      <CardContent>
        {isLoading ? (
          ...
          <EmptyState
            ...
            title="No tokens yet"
            description="Mint a source token to start shipping logs to this stream."
            actions={
              canManage ? (
                <Button onClick={() => setMintOpen(true)}>
                  <Plus className="size-4" />
      ...
      <ConfirmationModal
        ...
        message={
          <>
            ...
            token will immediately stop being able to send logs. This cannot be
            undone.
          </>
        }
        confirmText="Revoke"
